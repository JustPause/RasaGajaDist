import { c as createComponent, s as spreadAttributes, u as unescapeHTML, a as renderTemplate, d as createAstro, m as maybeRenderHead, b as addAttribute, g as renderScript, r as renderComponent } from '../chunks/astro/server_WWt00ON-.mjs';
import 'kleur/colors';
...
const $$AudioPlayer = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="w-full bg-accent rounded-2xl"> <div class="my-4 mx-8 flex flex-col sm:gap-8 items-center gap-4"> <div id="loading-spinner" class="flex justify-center items-center min-h-[200px] disabled"> <div class="w-8 h-8 border-4 border-t-transparent border-primary rounded-full animate-spin"></div> </div> <div id="full-audio-player" class="flex gap-2 w-fit p-1.5 rounded-lg flex-col hidden"> <div class="flex items-center"> <select id="list-books" class="w-full p-2 border rounded"> <option class="select-none" value="" disabled selected>Pasirink koki nori skyriu</option> ${list.map((post, index) => renderTemplate`<option${addAttribute(post, "value")} class="select-none"> ${index + 1}. ${post} </option>`)} </select> </div> <audio id="audio-player" src=""></audio> <div class="relative w-full flex justify-between items-center gap-4 mt-4"> <span id="current-time" class="time">0:00</span> <input type="range" id="seek-slider" max="100" value="0" class="w-full"> <span id="duration" class="time">0:00</span> </div> <div> <input type="range" id="volume-slider" max="100" value="100" class="w-full"> </div> <div class="flex justify-center sm:gap-1 w-fit *:rounded-lg *:p-1 *:sm:p-2 *:duration-300 *:hover:bg-secondary *:w-[20%] *:sm:w-[36px]"> <img${addAttribute(refresh.src, "src")} alt="refresh" id="refresh-btn"> <img${addAttribute(angle_double_left.src, "src")} alt="beginning" id="restart-btn"> <img${addAttribute(angle_left.src, "src")} alt="rewind" id="rewind-btn"> <img${addAttribute(play_pause.src, "src")} alt="toggle play/pause" id="toggle-btn"> <img${addAttribute(angle_right.src, "src")} alt="forward" id="forward-btn"> <img${addAttribute(angle_double_right.src, "src")} alt="end" id="end-btn"> </div> </div> </div> </div> ${renderScript($$result, "/home/justinas/Programming/RasoSvetaine/src/components/AudioPlayer.astro?astro&type=script&index=0&lang.ts")}`;
...
const $$audioKnygos = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const listOfAudio = await fetch(
    `https://rasagaja.lt/backendhidden/books/${audio_knygos}/`
  );
  const awaitlistOfAudio = await listOfAudio.json();
  return renderTemplate`${renderComponent($$result, "Layout", $$LayoutSecondary, { "title": audio_knygos.charAt(0).toUpperCase() + audio_knygos.slice(1).replaceAll("-", " ") }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "AudioPlayer", $$AudioPlayer, { "list": awaitlistOfAudio })} ` })} ${renderScript($$result, "/home/justinas/Programming/RasoSvetaine/src/pages/[audio_knygos].astro?astro&type=script&index=0&lang.ts")}`;
