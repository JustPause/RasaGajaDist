...
</p> </div> </div> </div> </div> </div> <div class="IndexPageOutsideListing"> <div class="mx-8 flex w-180 flex-wrap justify-center gap-8 xl:w-372 xl:flex-nowrap xl:gap-16"> ${tekstai.map((dat) => renderTemplate`<a class="bg-dark-primary-darker 2xs:w-72 bouttonAnimasion text-dark-text-light w-auto rounded-2xl p-4 text-center text-xl xl:w-auto xl:p-6"${addAttribute(dat.link, "href")}> ${dat.text.toUpperCase()} </a>`)} </div> </div> ` })}`;
