...
</p> <div class="flex flex-wrap items-stretch justify-center gap-4 *:uppercase"> ${skyreliai.map((dat) => renderTemplate`<a class="bg-dark-primary-darker 2xs:w-96 bouttonAnimasion text-dark-text-light h-auto w-full rounded-2xl p-4 text-center xl:p-6"${addAttribute(dat.link, "href")}> ${dat.text} </a>`)} </div> </div> </div> ` })}`;
