...
import 'kleur/colors';
import { a as $$Layout, $ as $$Image } from '../chunks/Layout_D4JteZMS.mjs';
...
const $$AudioIraai = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Audio \u012Fra\u0161ai" }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="IndexPageOutsideListing"> <div class="IndexPageInsideListing flex-col"> <p class="header">Nemunai teka į Drakono kalnus</p> <div class="flex flex-col items-center gap-8 md:block xl:flex xl:flex-row xl:items-start"> <div class="xs:float-left 2xs:block mr-0 flex w-full max-w-96 flex-col gap-8 md:mr-4 xl:flex"> ${renderComponent($$result2, "Image", $$Image, { "src": Audio_NEMUNAI_TEKA___DRAKONO_KALNUS, "alt": "Knygos vir\u0161elis", "class": "mb-8 rounded-2xl xl:mb-0" })} <a class="bg-dark-primary-darker bouttonAnimasion flex w-full max-w-96 items-center rounded-2xl p-5 xl:w-96" href="/klausyti-ištraukos"> <span class="material-symbols-rounded text-dark-text-light icons !text-6xl select-none">
play_arrow
</span> <p>Klausyti ištraukos</p> </a> </div> <div class="*:mb-8 xl:flex xl:flex-col xl:gap-8"> <p class="indent-8">
...
Knygą „NEMUNAI TEKA Į DRAKONO KALNUS“ 2025 m. išleido leidykla „KAUKO
          LAIPTAI“.
</p> <p class="indent-8">
...
</p> </div> ${renderComponent($$result2, "Image", $$Image, { "src": RASA_SU_VARNA, "alt": "Rasytoja nuotrauka", "class": "h-auto w-[26.5rem] rounded-2xl object-contain" })} </div> </div> ` })}`;
